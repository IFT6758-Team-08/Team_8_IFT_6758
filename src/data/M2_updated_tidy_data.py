...
def filter_df(df, games_metadata):
    ...
    new_df[['game_id', 'period', 'period_time',
            ...
            'coordinates_y', 'secondary_type']] = df[['game_id', 'about.period', 'about.periodTime',
...
def tidy_all_seasons(path, year, to_year):
    ...
    for y in range(year, to_year + 1):
        file_path_regular, file_path_playoff = path + "/" + str(y) + "_regular_season.json", path + "/" + str(
            y) + "_playoffs.json"
        output_regular_file = "M2_regular_" + str(y) + "_cleaned1.csv"
        output_playoff_file = "M2_playoff_" + str(y) + "_cleaned1.csv"

        # tidying the data for regular and playoff season in two dataframes
        print("tidying regular season ", year)
        df_regular = tidy_one_season(file_path_regular)
        # For test data
        # df_playoff = tidy_one_season(file_path_playoff)
        # print("tidying playoff ", year)
        # df_playoff = tidy_one_season(file_path_playoff)
        # FOr test data
        # df_playoff.to_csv(output_playoff_file, index=False, encoding='utf-8-sig')

        # Merging the dataframes and saving to csv
        # all_df = pd.concat([df_regular, df_playoff])
        df_regular.to_csv(output_regular_file, index=False, encoding='utf-8-sig')


def main():
    tidy_all_seasons("get data", 2016, 2016)
    tidy_all_seasons(".", 2016, 2016)
    tidy_all_seasons(".", 2017, 2017)
    tidy_all_seasons(".", 2018, 2018)
    tidy_all_seasons(".", 2019, 2019)
    data_2016 = pd.read_csv("M2_regular_2016_cleaned1.csv")
    data_2017 = pd.read_csv("M2_regular_2017_cleaned1.csv")
    data_2018 = pd.read_csv("M2_regular_2018_cleaned1.csv")
    data_2019 = pd.read_csv("M2_regular_2019_cleaned1.csv")
    # For test data
    # tidy_all_seasons(".", 2020, 2020)


    data = pd.concat([data_2016, data_2017, data_2018, data_2019])
    data.to_csv("M2_regular_all_cleaned1.csv", index=False, encoding='utf-8-sig')
